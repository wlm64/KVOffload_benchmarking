    ...
    for PROMPT_DATASET in "${DATASETS[@]}"; do
        for rate in "${REQUEST_RATES[@]}"; do
            for kt in "${KT[@]}"; do
                ...
                echo "---------------------------------------------"
                echo "Run=${run} | rate=${rate} | KT=${kt}"
                echo "---------------------------------------------"

                DATASET_PATH="./synthetic_prompts/synthetic_prompts_${PROMPT_DATASET}_KT_${kt}_memory.jsonl"

                # Log filenames (NOW include run)
                POWER_LOG="${LOG_DIR}/power_util_run_${run}_rate_${rate}_${PROMPT_DATASET}_KT_${kt}.csv"
                PCIE_LOG="${LOG_DIR}/pcie_util_run_${run}_rate_${rate}_${PROMPT_DATASET}_KT_${kt}.csv"
                SCHEDULER_LOG="${LOG_DIR}/scheduler_trace_run_${run}_rate_${rate}_${PROMPT_DATASET}_KT_${kt}.log"
                RESULT_LOG="${LOG_DIR}/benchmark_results_run_${run}_KT_${kt}.log"
